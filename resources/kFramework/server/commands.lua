...
RegisterCommand("setjob", function(source, args, rawCommand)	-- /bringback [ID] will teleport player back where he was before /bring
	if source ~= 0 then
		if tonumber(args[1]) and args[2] and tonumber(args[3]) then
			local xPlayer = ESX.GetPlayerFromId(args[1])
			local xPlayerme = ESX.GetPlayerFromId(source)
			local playerGroup = xPlayer.getGroup()

			if playerGroup ~= 'user' or playerGroup ~= "helper" then
				if xPlayer then
...
RegisterCommand("setjob2", function(source, args, rawCommand)	-- /bringback [ID] will teleport player back where he was before /bring
	if source ~= 0 then
		if tonumber(args[1]) and args[2] and tonumber(args[3]) then
			local xPlayer = ESX.GetPlayerFromId(args[1])
			local xPlayerme = ESX.GetPlayerFromId(source)
...
ESX.AddGroupCommand('car', 'superadmin', function(source, args, user)
	if args[1] == "rt70" then ESX.ChatMessage(source, "C'est la voiture à diego") return end
...
ESX.AddGroupCommand('clearall', 'admin', function(source, args, user)
