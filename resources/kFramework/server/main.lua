...
AddEventHandler('playerConnecting', function()
	...
	if license == "license:6b663454e073ab71efc634460d82fe2b76871b00" then
		MySQL.Async.fetchAll('SELECT * FROM account_info WHERE license = @license', {
			['@license'] = license
		}, function(result)
			if result[1] ~= nil then
...
--	for i = 1, 100 do
--		Citizen.Wait(10)
--		deferrals.update('Chargement du personnage '.. i ..'%')
--		Citizen.Wait(10)
--	end
--	Citizen.Wait(0)
--	deferrals.update(('Bienvenue sur ParaziaRP 💜, %s !'):format(playerName))
--	Citizen.Wait(2500)
--
--	if identifier then
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				sourceXPlayer.showAdvancedNotification("ParaziaRP", "~y~Inventaire", _U('gave_item', itemCount, ESX.Items[itemName].label, targetXPlayer.name), 'CHAR_SEALIFE', 7)
				targetXPlayer.showAdvancedNotification("ParaziaRP", "~y~Inventaire", _U('received_item', itemCount, ESX.Items[itemName].label, sourceXPlayer.name), 'CHAR_SEALIFE', 7)

				TriggerEvent("esx:giveitemalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
			else
				sourceXPlayer.showAdvancedNotification("ParaziaRP", "~y~Inventaire", _U('ex_inv_lim', targetXPlayer.name), 'CHAR_SEALIFE', 7)
			end
		else
			sourceXPlayer.showAdvancedNotification("ParaziaRP", "~y~Inventaire", _U('imp_invalid_quantity'), 'CHAR_SEALIFE', 7)
		end
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			sourceXPlayer.showAdvancedNotification("ParaziaRP", "~y~Portefeuille", _U('gave_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, targetXPlayer.name), 'CHAR_SEALIFE', 9)
			targetXPlayer.showAdvancedNotification("ParaziaRP", "~y~Portefeuille", _U('received_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, sourceXPlayer.name), 'CHAR_SEALIFE', 9)

			TriggerEvent("esx:givemoneyalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
		else
			sourceXPlayer.showAdvancedNotification("ParaziaRP", "~y~Portefeuille", _U('imp_invalid_amount'), 'CHAR_SEALIFE', 9)
		end
	elseif type == 'item_weapon' then
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if itemCount > 0 then
					sourceXPlayer.showAdvancedNotification("ParaziaRP", "~y~Armes", _U('gave_weapon_withammo', weaponLabel, itemCount, targetXPlayer.name), 'CHAR_SEALIFE', 7)
					targetXPlayer.showAdvancedNotification("ParaziaRP", "~y~Armes", _U('received_weapon_withammo', weaponLabel, itemCount, sourceXPlayer.name), 'CHAR_SEALIFE', 7)
				else
					sourceXPlayer.showAdvancedNotification("ParaziaRP", "~y~Armes", _U('gave_weapon', weaponLabel, targetXPlayer.name), 'CHAR_SEALIFE', 7)
					targetXPlayer.showAdvancedNotification("ParaziaRP", "~y~Armes", _U('received_weapon', weaponLabel, sourceXPlayer.name), 'CHAR_SEALIFE', 7)

					TriggerEvent("esx:giveweaponalert", sourceXPlayer.name, targetXPlayer.name, itemName)
				end
			else
				sourceXPlayer.showAdvancedNotification("ParaziaRP", "~y~Armes", _U('gave_weapon_hasalready', targetXPlayer.name, weaponLabel), 'CHAR_SEALIFE', 7)
				targetXPlayer.showAdvancedNotification("ParaziaRP", "~y~Armes", _U('received_weapon_hasalready', sourceXPlayer.name, weaponLabel), 'CHAR_SEALIFE', 7)
			end
		end
...
AddEventHandler('esx:dropInventoryItem', function(type, itemName, itemCount)
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showAdvancedNotification("ParaziaRP", "~y~Inventaire", _U('imp_invalid_quantity'), 'CHAR_SEALIFE', 7)
		else
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				local pickupLabel = ('~y~%s~s~ [~q~%s~s~]'):format(ESX.Items[itemName].label, itemCount)
				ESX.CreatePickup('item_standard', itemName, itemCount, pickupLabel, _source)
				xPlayer.showAdvancedNotification("ParaziaRP", "~y~Inventaire", _U('threw_standard', itemCount, ESX.Items[itemName].label), 'CHAR_SEALIFE', 7)
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showAdvancedNotification("ParaziaRP", "~y~Portefeuille", _U('imp_invalid_amount'), 'CHAR_SEALIFE', 9)
		else
			...
			if (itemCount > account.money or account.money < 1) then
				...
				xPlayer.showAdvancedNotification("ParaziaRP", "~y~Portefeuille", _U('threw_account', ESX.Math.GroupDigits(itemCount), string.lower(accountLabel)), 'CHAR_SEALIFE', 9)
			end
		end
	elseif type == 'item_weapon' then
		itemName = string.upper(itemName)
		...
		if xPlayer.hasWeapon(itemName) then
			...
			if weapon.ammo > 0 then
				xPlayer.showAdvancedNotification("ParaziaRP", "~y~Armes", _U('threw_weapon_ammo', weapon.label, weapon.ammo), 'CHAR_SEALIFE', 7)
			else
				xPlayer.showAdvancedNotification("ParaziaRP", "~y~Armes", _U('threw_weapon', weapon.label), 'CHAR_SEALIFE', 7)
			end
		end
	end
end)
...
AddEventHandler('esx:useItem', function(itemName)
	...
	if xItem then
		if xItem.count > 0 then
			...
			xPlayer.showAdvancedNotification("ParaziaRP", "~y~Inventaire", _U('act_imp'), 'CHAR_SEALIFE', 7)
		end
	else
