...
function ESX.ShowNotification(msg, flash, saveToBrief, hudColorIndex, title, subject, icon)
	...
	if title == nil then title = "ParaziaRP" end
	if subject == nil then subject = "ANNONCE" end
	if icon == nil then icon = "message" end

	exports.ParaziaNotif:Send(msg, nil, nil, true, nil, title, subject, icon)
end
...
function ESX.ShowAdvancedNotification(title, subject, msg, banner, timeout, icon)
	...
	exports.ParaziaNotif:SendAdvanced(msg, subject, title, banner, nil, nil, true, nil, icon)
end
...
RegisterNetEvent('esx:drawCenterText')
AddEventHandler('esx:drawCenterText', function(msg, time)
	DrawCenterText(msg, time)
end)
