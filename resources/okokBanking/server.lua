...
local Webhook = 'https://discord.com/api/webhooks/1111349701510443089/OVldTHJA4hWeKH6LzlPZAOjeJCWnH5F2XbiL1SIFG3Tu-9kXlxhdAq0sZfridlx38p3-'
	...
	if xPlayer.identifier ~= targetIdentifier then
		if amount <= playerMoney then
			if xTarget ~= nil then
				...
				TriggerClientEvent('okokBanking:updateTransactions', _source, xPlayer.getAccount('bank').money, xPlayer.getAccount("cash").money)
...
AddEventHandler("okokBanking:TransferMoneyToSociety", function(amount, ibanNumber, societyName, society)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactionsSociety', _source, xPlayer.getAccount("cash").money)
		TriggerClientEvent('okokBanking:updateTransactions', _source, xPlayer.getAccount('bank').money, xPlayer.getAccount("cash").money)
		TriggerClientEvent('okokBanking:updateMoney', _source, xPlayer.getAccount('bank').money, xPlayer.getAccount("cash").money)
...
AddEventHandler("okokBanking:GiveCC", function()
	...
	if Config.CreditCardPrice <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', xPlayer.source, xPlayer.getAccount('bank').money, xPlayer.getAccount("cash").money)
