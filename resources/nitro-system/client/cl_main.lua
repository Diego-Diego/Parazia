...
AddEventHandler('nitro:start', function()
	...
	if IsPedSittingInAnyVehicle(playerPed) then
		ESX.ShowNotification('You cant install the nitro bottle inside the vehicle!')
	else
		if DoesEntityExist(vehicle) and IsPedOnFoot(playerPed) then
			...
			Citizen.CreateThread(function()
				...
				if lib.progressCircle({
					...
					startNitro()
					TriggerServerEvent('nitro:removeInventoryItem','zetony', 1)
					ESX.ShowNotification('You Installed a Nitro Bottle!')
				else 
					ClearPedTasksImmediately(playerPed) 
			...
			ESX.ShowNotification('No Vehicle(s) Nearby!')
		end
	end
end)
