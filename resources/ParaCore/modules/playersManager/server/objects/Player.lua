...
function Player:notify(message)
    ParaziaRPServerUtils.toClient("esx:showNotification", self.source, message)
end
...
function Player:setAddonCache(index, value, shouldTriggerAssociedEvent)
    ...
    if shouldTriggerAssociedEvent then
        ParaziaRPSPlayersManager.addonCache[index][3](self)
    end
end
...
function Player:playMusicEvent(music)
    if not playingMusics[music] then
        ...
        ParaziaRPServerUtils.toClient("fivemPlayMusic", self.source, music)
    end
end
...
function Player:stopMusicEvent(music)
    if playingMusics[music] then
        ...
        ParaziaRPServerUtils.toClient("fivemStopMusic", self.source, music)
    end
end
...
function Player:incrementSessionTimePlayed()
    ...
    if self.session[1] == 30 then
        ParaziaRP.toInternal("onPlayerSessionMidElapsed", self.source)
    end
    if self.session[1] > 60 then
        self.session[1] = 0
        self.session[2] = self.session[2] + 1
        ParaziaRP.toInternal("onPlayerSessionHourElapsed", self.source)
    end
end
