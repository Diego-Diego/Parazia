...
ParaziaRP.newWaitingThread(100, function()
    badgeFromValue = {
...
ParaziaRP.netRegisterAndHandle("openCloackroom", function(job)
                ...
                RageUI.ButtonWithStyle("Tenue de travail", "Vous permets d'enfiler votre tenue de travail", { RightBadge = badgeFromValue[isWithWorkingClothes] }, true, function(_, _, s)
                    if s and not isWithWorkingClothes then
                        ...
                        TriggerEvent('skinchanger:getSkin', function(skin)
                            TriggerEvent('skinchanger:loadClothes', skin, ParaziaRPSharedCustomJobs[job].clothes[Job.grade_name][byIndex[skin.sex == 0]])
                        end)
                    end
                end)
            end, function()
