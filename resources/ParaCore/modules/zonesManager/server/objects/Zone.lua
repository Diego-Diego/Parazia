...
setmetatable(Zone, {
    __call = function(_, location, type, color, onInteract, helpText, drawDist, itrDist, restricted, baseAllowed)
        ...
        self.zoneID = (#ParaziaRPSZonesManager.list + 1)
        ...
        ParaziaRPSZonesManager.list[self.zoneID] = self
        return self
    end
})
...
function Zone:interact(source)
    ...
    ParaziaRPServerUtils.trace(("%s a intéragit avec la zone n°%s"):format(GetPlayerName(source),self.zoneID), ParaziaRPPrefixes.zones)
end
