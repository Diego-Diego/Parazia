...
ParaziaRP.netHandle("esxloaded", function()
    ParaziaRPClientUtils.toServer("requestPredefinedZones")
    while true do
        local interval = 500
                ...
                if dist <= zone.distances[2] then
                    ...
                    if IsControlJustPressed(0, 51) then
                        if not zones.cooldown then
                            zones.cooldown = true
                            ParaziaRPClientUtils.toServer("interactWithZone", zone.id)
                            ParaziaRP.newWaitingThread(450, function()
                                zones.cooldown = false
                            end)
                        end
                    end
...
ParaziaRP.netRegisterAndHandle("newMarker", function(zone)
    zones.list[zone.id] = zone
end)

ParaziaRP.netRegisterAndHandle("delMarker", function(zoneID)
    zones.list[zoneID] = nil
end)

ParaziaRP.netRegisterAndHandle("cbZones", function(zoneInfos)
