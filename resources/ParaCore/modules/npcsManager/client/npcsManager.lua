...
ParaziaRP.netHandle("esxloaded", function()
    ParaziaRPClientUtils.toServer("requestPredefinedNpcs")
            ...
            for npcId, npc in pairs(npcs.list) do
                ...
                if not npcs.list[npcId].npcHandle then
                    if #(pos - npcPos) <= genDist then
                        ...
                        if npc.frozen then
                            ParaziaRP.newWaitingThread(1500, function()
...
ParaziaRP.netRegisterAndHandle("npcPlaySound", function(npcId, speech, param)
...
---@param npc Npc
ParaziaRP.netRegisterAndHandle("newNpc", function(npc)
    npcs.list[npc.id] = npc
end)

ParaziaRP.netRegisterAndHandle("delNpc", function(npcId)
...
ParaziaRP.netRegisterAndHandle("cbNpcs", function(incomingNpcs)
