...
setmetatable(Npc, {
    __call = function(_, model, ai, frozen, position, animation, restricted, allowed)
        ...
        self.id = #ParaziaRPSNpcsManager.list + 1
        self.model = model
        self.ai = ai
        self.frozen = frozen
        ...
        ParaziaRPSNpcsManager.list[self.id] = self
        return self;
    end
})
...
function Npc:playSpeech(speech, param)
    if self:isRestricted() then
        for source, _ in pairs(self.allowed) do
            ParaziaRPServerUtils.toClient("npcPlaySound", source, self.id, speech, param)
        end
    else
        ParaziaRPServerUtils.toAll("npcPlaySound", self.id, speech, param)
    end
end
