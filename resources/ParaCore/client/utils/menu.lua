...
ParaziaRPMenuUtils.inputBox = function(TextEntry, ExampleText, MaxStringLenght, isValueInt)
...
ParaziaRPMenuUtils.okIfDef = function(value, isMale)
    local e = ""
    if not isMale then e = "e" end
    if not value or value == "" then return "" else return ("~s~: ~g~Défini%s"):format(e) end
end

ParaziaRPMenuUtils.valueNotDefault = function(value, add)
