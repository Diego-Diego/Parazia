...
local cat, desc = "plageFridge", "~y~Réfrigérateur de la plage"
local sub = function(str)
    return cat .. "_" .. str
end

ParaziaRP.netRegisterAndHandle("plageCbItem", function()
    awaitingServerUpdate = false
end)

ParaziaRP.netRegisterAndHandle("plageOpenFrigo", function(items)
                ...
                for k,v in pairs(items) do
                    RageUI.ButtonWithStyle(("Prendre \"~y~%s~s~\""):format(v.label), "Appuyez pour prendre du stock", {RightBadge = RageUI.BadgeStyle.Franklin}, not awaitingServerUpdate, function(_,_,s)
                        if s then
                            ...
                            ParaziaRPClientUtils.toServer("plageRequestItem", k)
                        end
                    end)
                end
            end, function()
