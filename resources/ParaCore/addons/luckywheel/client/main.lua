...
local function startSpin()
    VisionRP.newThread(function()
        ...
        VisionRPClientUtils.toServer("luckywheelRequestFinalPrice", ESX.Game.GetVehicleProperties(veh))
        --Citizen.Wait(5000);
        isRolling = false;
    end)
end
...
RegisterNetEvent("luckywheelCbCurrentVehicle")
AddEventHandler("luckywheelCbCurrentVehicle", function(vehicle)
    currentVehicleRewardModel = vehicle
end)

RegisterNetEvent("luckywheelVehicleChange")
AddEventHandler("luckywheelVehicleChange", function(vehicle)
    currentVehicleRewardModel = vehicle
    removeAllWheelShits()
end)
...
AddEventHandler("esxloaded", function()
    VisionRPClientUtils.toServer("luckywheelRequestCurrentVehicle")
                ...
                if dist > 30.0 then
                    ...
                    if dist <= 1.0 and not isRolling then
                        if not currentTurn then
                            AddTextEntry("VisionRPRoue", "Vous n'avez pas la permission de faire tourner la roue ! Allez vous renseigner auprès du vendeur de tickets.")
                        else
                            AddTextEntry("VisionRPRoue", "Appuyez sur ~INPUT_CONTEXT~ pour faire tourner la roue")
                        ...
                        DisplayHelpTextThisFrame("VisionRPRoue", 0)
                    end
                end
            else
                if dist < 30.0 then
