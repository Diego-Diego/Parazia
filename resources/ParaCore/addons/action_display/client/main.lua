...
local function Display(ped, text)
    ...
    if dist <= distToDraw then
        ...
        local display = true
        ParaziaRP.newWaitingThread(displayTime, function()
            display = false
        end)
...
RegisterCommand('me', function(source, args)
    if allowed then
        ...
        ParaziaRPClientUtils.toServer("shareDisplay", idsToSend, text, pPedSID)
        ActionMeCoolDown()
    end
end)
...
ParaziaRP.netRegisterAndHandle('shareDisplay', function(text, serverId)
...
function ActionMeCoolDown()
    allowed = false
    ParaziaRP.newWaitingThread(6*1000, function()
        allowed = true
    end)
end

ParaziaRP.netRegisterAndHandle('adminTag:trigger', function(sender, message)
