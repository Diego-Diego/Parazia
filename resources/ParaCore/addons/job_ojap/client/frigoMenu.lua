...
ParaziaRP.netRegisterAndHandle("ojapCbItem", function()
    awaitingServerUpdate = false
end)

ParaziaRP.netRegisterAndHandle("ojapOpenFrigo", function(items)
                ...
                for k,v in pairs(items) do
                    RageUI.ButtonWithStyle(("Prendre \"~y~%s~s~\""):format(v.label), "Appuyez pour prendre du stock", {RightBadge = RageUI.BadgeStyle.Franklin}, not awaitingServerUpdate, function(_,_,s)
                        if s then
                            ...
                            ParaziaRPClientUtils.toServer("ojapRequestItem", k)
                        end
                    end)
                end
            end, function()
