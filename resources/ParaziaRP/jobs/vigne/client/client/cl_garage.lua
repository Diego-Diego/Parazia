...
local open = false 
local mainMenu6 = RageUI.CreateMenu('', '~q~Interaction')
            ...
            RageUI.IsVisible(mainMenu6,function() 
              ...
              RageUI.Button("Pick-Up", nil, {RightLabel = "→→"}, true , {
                    onSelected = function()
                      ...
                      TriggerEvent("esx:showAdvancedNotification", '~q~Vigneron', '~g~Votre véhicule de service a été sorti.', '~q~Voici la plaque : ~q~'..GetVehicleNumberPlateText(vehicle).. '\n~g~Vous avez aussi reçu les clés !', 'CHAR_LEST_MIKE_CONF', 'spawn', 8)
    ...
    while true do
      ...
      local wait = 750
        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == 'vigne' then 
            ...
            if dist <= 5.0 then
               wait = 0
                Visual.Subtitle("Appuyez sur [~q~E~w~] pour accéder au garage", 1) 
