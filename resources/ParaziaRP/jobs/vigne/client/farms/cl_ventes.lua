...
local open = false 
local mainMenu = RageUI.CreateMenu('', '~q~Vigne')
	...
	if open then 
		...
		while open do 
		   RageUI.IsVisible(mainMenu,function() 
			...
			RageUI.Button("Vendre Jus de raisin", nil, {RightLabel = "~q~25$"}, true , {
				onSelected = function()
                    TriggerServerEvent('selljusraisin')
				end
			}, subMenu)

			RageUI.Button("Vendre Grand Cru", nil, {RightLabel = "~q~38$"}, true , {
				onSelected = function()
    ...
    while true do
      ...
      local wait = 750
        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == 'vigne' then 
            ...
            if dist <= 10.0 then
               wait = 0
                Visual.Subtitle("Appuyer sur ~q~[E]~s~ pour ~q~intéragir", 1) 
