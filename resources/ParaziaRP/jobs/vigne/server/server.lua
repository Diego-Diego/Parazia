...
AddEventHandler('Ouvre:vigne', function()
	...
	if xPlayer.job.name == 'vigne' then
		for i=1, #xPlayers, 1 do
			...
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vigne', '~g~Annonce', '~g~Les vigne sont ouvert ! Venez achetez votre vin et jus de raisin !', 'CHAR_LEST_MIKE_CONF', 8)
		end
	end
end)
...
AddEventHandler('Ferme:vigne', function()
	...
	if xPlayer.job.name == 'vigne' then
		for i=1, #xPlayers, 1 do
			local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vigne', '~g~Annonce', '~q~Les vignes ferme pour le moment !', 'CHAR_LEST_MIKE_CONF', 8)
		end
	end
end)
...
AddEventHandler('Recru:vigne', function()
	...
	if xPlayer.job.name == 'vigne' then
		for i=1, #xPlayers, 1 do
			local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Vigne', '~g~Annonce', '~y~Recrutement en cours, rendez-vous au vigne !', 'CHAR_LEST_MIKE_CONF', 8)
		end
	end
end)
	...
	if #(coords - vigne) < ZoneSize / 2 then
		if xPlayer.job.name == 'vigne' then
			if jus_raisin > 250 then
				TriggerClientEvent('esx:showNotification', source, '~q~Il semble que tu ne puisses plus porter de jus de raisin...')
			elseif raisin < 5 then
				TriggerClientEvent('esx:showNotification', source, '~q~Pas assez de jus de raisin pour traiter...')
			else
	...
	if #(coords - vigne) < ZoneSize / 2 then
		if xPlayer.job.name == 'vigne' then
			...
			if jus_raisin > 0 then
				...
				xPlayer.addAccountMoney('cash', 15)
				societyAccount.addMoney(15)
				TriggerClientEvent('esx:showNotification', xPlayer.source, "~g~Vous avez gagner ~q~15$~g~ pour chaque vente d'un jus de raisin")
				TriggerClientEvent('esx:showNotification', xPlayer.source, "~g~La société gagne ~q~15$~g~ pour chaque vente d'un jus de raisin")
			else 
	...
	if #(coords - vigne) < ZoneSize / 2 then
		if xPlayer.job.name == 'vigne' then
			...
			if grand_cru > 0 then
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, "~g~Vous avez gagner ~q~25$~g~ pour chaque vente d'un grand cru")
				TriggerClientEvent('esx:showNotification', xPlayer.source, "~g~La société gagne ~q~25$~g~ pour chaque vente d'un grand cru")
			else 
				TriggerClientEvent('esx:showNotification', xPlayer.source, "Vous n'avez plus rien à vendre")
			end
		end
