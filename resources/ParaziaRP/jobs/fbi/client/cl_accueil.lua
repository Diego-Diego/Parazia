...
local open = false 
local mainMenu = RageUI.CreateMenu('', 'fbi') 
local subMenu1 = RageUI.CreateMenu('', 'fbi') 
...
function OpenMenuAccueilfbi() 
    if open then 
        ...
        RageUI.Button("Appel Un Policier a l'accueil", nil, {RightLabel = "→→"}, not codesCooldown5 , {
            onSelected = function()
            codesCooldown5 = true 
        TriggerServerEvent('Appel:fbi')
        ESX.ShowNotification('~g~Votre message a bien été envoyé aux fbir.')
            ...
            RageUI.Button("Valider la Demande", "~o~Merci de respecter les ambulancier.", { Color = {BackgroundColor = { 76, 175, 80, 50}} }, true, {
                onSelected = function()
                    if (nomprenom == nil or nomprenom == '') then
                        ESX.ShowNotification('~q~Vous n\'a pas rempli ton Nom/Prénom')
                    elseif (numero == nil or numero == '') then
                        ESX.ShowNotification('~q~Vous n\'a pas rempli ton Numéro')
                    elseif (heurerdv == nil or heurerdv == '') then
                        ESX.ShowNotification('~q~Vous n\'a pas rempli l\'heure de Rendez-vous')
                    elseif (rdvmotif == nil or rdvmotif == '' or rdvmotif == "Motif") then
                        ESX.ShowNotification('~q~Vous n\'a pas rempli le motif du rendez-vous')
                else
                    RageUI.CloseAll()
                    TriggerServerEvent("Rdv:fbiMotif", nomprenom, numero, heurerdv, rdvmotif)
                    ESX.ShowNotification("~q~fbi~s~\nDemande de Rendez-vous envoyée")
                    nomprenom = nil
                    numero = nil
                    heurerdv = nil
                    rdvmotif = nil
...
Citizen.CreateThread(function()
    while true do 
        local wait = 750
            ...
            for k in pairs(cfg_fbi.Position.Accueil) do 
                local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
                local pos = cfg_fbi.Position.Accueil
                local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, pos[k].x, pos[k].y, pos[k].z)
  
                if dist <= 2.0 then 
                    wait = 0
                    Visual.Subtitle(cfg_fbi.TextAccueil, 1)
