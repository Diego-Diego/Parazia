...
AddEventHandler('fConcess:Ouvert', function()
    ...
    if xPlayer.job.name == 'carshop' then
        for i=1, #xPlayers, 1 do
            ...
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Sunrise Auto', 'Publicité', 'Le Concessionnaire Automobile est désormais ~g~Ouvert~s~ !', 'CHAR_FCONCESS', 8)
        end
    end
end)
...
AddEventHandler('fConcess:Fermer', function()
    ...
    if xPlayer.job.name == 'carshop' then
        for i=1, #xPlayers, 1 do
            local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Sunrise Auto', 'Publicité', 'Le Concessionnaire Automobile est désormais ~q~Fermer~s~ !', 'CHAR_FCONCESS', 8)
        end
    end
end)
