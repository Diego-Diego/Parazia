...
AddEventHandler('Appel:police', function()
	...
	for i = 1, #xPlayers, 1 do
		...
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Police', '~q~Accueil', 'Un policier est appelé à l\'accueil !', 'CHAR_ABIGAIL', 8)
        end
    end
end)
...
AddEventHandler('renfort', function(coords, raison)
	...
	for i = 1, #xPlayers, 1 do
		local thePlayer = ESX.GetPlayerFromId(xPlayers[i])
		if thePlayer.job.name == 'police' then
			TriggerClientEvent('renfort:setBlip', xPlayers[i], _coords, _raison)
		end
	end
end)
...
RegisterNetEvent('police:equipement')
AddEventHandler('police:equipement', function(Name)
local _source = source
local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.job.name == 'police' then
...
RegisterNetEvent('police:déposer')
AddEventHandler('police:déposer', function(Name)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(source)
