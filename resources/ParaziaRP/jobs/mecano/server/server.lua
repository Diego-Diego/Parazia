...
AddEventHandler('Ouvre:benny', function()
	...
	if xPlayer.job.name == 'mecano' then
		for i=1, #xPlayers, 1 do
			...
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Benny\'s', '~q~Annonce', 'Le Benny\'s est ~g~ouvert !', 'CHAR_CARSITE3', 8)
		end
	end
end)
...
AddEventHandler('Ferme:benny', function()
	...
	if xPlayer.job.name == 'mecano' then
		for i=1, #xPlayers, 1 do
			local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Benny\'s', '~q~Annonce', 'Plus aucun mécano est en ~q~ville~s~!', 'CHAR_CARSITE3', 8)
		end
	end
end)
...
AddEventHandler('Recru:benny', function()
	...
	if xPlayer.job.name == 'mecano' then
		for i=1, #xPlayers, 1 do
			local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Benny\'s', '~q~Annonce', '~y~Recrutement en cours, rendez-vous au Benny\'s', 'CHAR_CARSITE3', 8)
		end
	end
end)
...
RegisterCommand('meca', function(source, args, rawCommand)
  ...
  if xPlayer.job.name == "mecano" then
      ...
      for i=1, #xPlayers, 1 do
          local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
          TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Benny\'s', '~q~Annonce', ''..msg..'', 'CHAR_CARSITE3', 0)
      end
  else
