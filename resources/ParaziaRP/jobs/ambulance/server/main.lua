MySQL.ready(function()
	MySQL.Async.execute('DELETE FROM `paraziarp_isdead`', {}, function()
	end)
end)
    ...
    if target == -1 then
        ...
        if xPlayer.job.name == 'ambulance' then
            if xPlayer.getInventoryItem('medikit').count >= 1 then
                ...
                xPlayer.showNotification('~w~~q~Vous n\'avez pas les outils nécéssaire.')
            end
        else
            DropPlayer(source, 'Désynchronisation avec le serveur ou detection de Cheat')
            return
...
AddEventHandler('EMS:annonce', function(value)
    ...
    if xPlayer.job.name == 'ambulance' then
        if value == 'open' then
            TriggerClientEvent('esx:showAdvancedNotification', -1, 'Informations ParaziaRP', '~q~Ambulance', 'Les ~q~EMS  ~w~sont ~g~disponible ~w~en ville !', 'CHAR_SEALIFE', 7)
        elseif value == 'close' then
            TriggerClientEvent('esx:showAdvancedNotification', -1, 'Informations ParaziaRP', '~q~Ambulance', 'Les ~q~EMS  ~w~sont ~q~indisponible ~w~en ville !', 'CHAR_SEALIFE', 7)
        else
...
AddEventHandler('ewen:RetreiveIsDead', function()
    ...
    MySQL.Async.fetchAll('SELECT * FROM `paraziarp_isdead` WHERE `license` = @license', {
...
AddEventHandler('EMS:InformPatient', function(target)
    if target == -1 then
        ...
        xPlayer.showNotification(' ~w~Un medecin est en route (~q~'..math.floor(distance)..'m~w~)')
    end
end)
...
AddEventHandler('ewen:RespawnVIP', function()
    ...
    TriggerClientEvent('EMS:ReviveClientPlayer', source)
    --PlayerIsDead[xPlayer.source].isDead = 0
    xPlayer.showNotification(' ~w~Vous avez été réanimer à l\'hôpital. Merci de votre soutien !(~q~VIP~s~)')
end)]]
...
AddEventHandler('playerDropped', function (reason)
    ...
    if PlayerIsDead[xPlayer.source] then
        if PlayerIsDead[xPlayer.source].isDead == 1 then
            MySQL.Async.fetchAll('SELECT * FROM `paraziarp_isdead` WHERE `license` = @license', {
                ...
                if result[1] then
                    ...
                    MySQL.Async.execute('INSERT INTO `paraziarp_isdead` (license) VALUES (@license)', {
                        ['@license'] = xPlayer.identifier,
                    }, function()
                    end)
                end
        ...
        MySQL.Async.fetchAll('SELECT * FROM `paraziarp_isdead` WHERE `license` = @license', {
            ...
            if result[1] then
                MySQL.Async.execute('DELETE FROM `paraziarp_isdead` WHERE `license` = @license', {
                    ['@license'] = xPlayer.identifier
                })
            end
        end)
