...
RegisterNetEvent('ewen:PlayerIsDead', function()
    SetEntityHealth(PlayerPedId(), 0)
end)

local ReportListTable = {}

RegisterNetEvent('ewen:UpdateTableSignalEms', function(table)
...
function OpenReportListEms()
    ...
    while menu do
        ...
        RageUI.IsVisible(menu, function()
            RageUI.Separator('~q~Appel en Attente')
            ...
            RageUI.Separator('~q~Appel en Cours')
            for k,v in pairs(ReportListTable) do
                if v.status == 1 then
                    RageUI.Button('Appel N*'..v.numbers, 'Description : '..v.raison..'\nAppel pris par ~q~'..v.EMSName, {RightLabel = 'Fait à '..v.heures..'h'..v.minutes.. 'm'..v.secondes..'s'}, true, {
                        onSelected = function() 
                            SrcSelected = v.src
                            AppelsSelected = v.numbers
                            Wait(500)
        ...
        RageUI.IsVisible(OpenSelectedAppel, function()
            RageUI.Separator('')
            RageUI.Separator('Appel N*~q~'..AppelsSelected)
            print(ReportListTable[SrcSelected].status)
            if ReportListTable[SrcSelected].status == 1 then
                StatusText = 'Pris par ~q~'..ReportListTable[SrcSelected].EMSName
            else 
                StatusText = '~g~En Attente'
            end
