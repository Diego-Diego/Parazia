...
RegisterCommand('jobsbuilder', function(source, args)
    ...
    if xPlayer.getGroup() == '_dev' then
        MowgliServerUtils.toClient('ewen:jobsbuilder', source)
    end
end)

RegisterServerEvent('Mowgli:CreateFarmEntreprise')
AddEventHandler('Mowgli:CreateFarmEntreprise', function(namejob, labeljob, namerecolteitem, labelrecolteitem, PositionRecolte, nametraitementitem, labeltraitementitem, PositionTraitement, PositionVente, PositionCoffreEntreprise)
    ...
    TriggerClientEvent('esx:showNotification', source, '~q~Mowgli ~w~~n~Le Job à été crée avec succès, Attendez le prochain reboot.')
end)
...
RegisterServerEvent('Mowgli:initFarmSociety')
AddEventHandler('Mowgli:initFarmSociety', function()
    TriggerClientEvent('Mowgli:SendEntrepriseFarmList', source, EntrepriseFarmList)
end)
		...
		if type == 1 then -- Recolte
			if ListeItemRecolt[itemRecolte] == nil then
				...
				if ListeItemRecolt[itemRecolte].name == itemRecolte then
					...
					TriggerEvent('Mowglilpb_fantadmin:ban', source, source, 'Tricher est interdit ( Activité Légal )', 0)
				end
			end
			...
			if ListeItemTraitement[ItemTraitement] == nil then
				...
				if ListeItemTraitement[itemTraitement] == itemTraitement then
					...
					if Quantity <= 0 then
						...
						SocietyCache[society].money = SocietyCache[society].money + 125
                        xPlayer.addAccountMoney('cash', 125)
						Activity(source, itemRecolte, type, ItemTraitement, society)
					end
				else
					DropPlayer(source, 'Désynchronisation avec les Entreprises de farm')
