        ...
        if #(GetEntityCoords(GetPlayerPed(source)) - position) < 8 then
            if xPlayer.getInventoryItem(item).count >= qty then
                ...
                if itemExiste then 
                    ...
                    TriggerClientEvent("SunriseESX:showNotification", xPlayer.source, "~g~Information~s~\nVous avez déposer ~q~x"..qty.." "..label.."~s~.")
                else
                ...
                TriggerClientEvent("esx:showNotification", xPlayer.source, "Information\n~q~Vous n'avez pas cette quantité")
            end
        else
            --TriggerEvent('Mowglilpb_fantadmin:ban', source, source, 'Tricher est interdit (ajout d\'un item dans une societer)', 0)
        end
    else
        --TriggerEvent('Mowglilpb_fantadmin:ban', source, source, 'Tricher est interdit (ajout d\'un item dans une societer)', 0)
    end
        ...
        if #(GetEntityCoords(GetPlayerPed(source)) - position) < 8 then
            ...
            if itemExiste then 
                if SocietyCache[society].data[item].count - qty < 0 then
                    ...
                    TriggerClientEvent("esx:showNotification", xPlayer.source, "~g~Information~s~\nVous avez pris ~q~x"..qty.." "..label.."~s~.")
                else
                    SocietyCache[society].data[item].count = SocietyCache[society].data[item].count - qty
                    xPlayer.addInventoryItem(item, qty)
                    TriggerClientEvent("esx:showNotification", xPlayer.source, "~g~Information~s~\nVous avez pris ~q~x"..qty.." "..label.."~s~.")
            ...
            --TriggerEvent('Mowglilpb_fantadmin:ban', source, source, 'Tricher est interdit (remove d\'un item dans une societer)', 0)
        end
    else
        --TriggerEvent('Mowglilpb_fantadmin:ban', source, source, 'Tricher est interdit (remove d\'un item dans une societer)', 0)
    end
    ...
    if xJob.name == society then
        if action == "deposit" then
            if xPlayer.getAccount('cash').money >= money then
                ...
                TriggerClientEvent("esx:showNotification", xPlayer.source, "~g~Information~s~\nVous avez déposer ~q~"..money.."$~s~ dans votre coffre.")
            else
                TriggerClientEvent("esx:showNotification", xPlayer.source, "Information\n~q~Vous n'avez pas cette quantité")
            end
            --local webhook = "https://discord.com/api/webhooks/834787175026655312/f1hhgGxYbJ09rI55KJ-vlpyDJiutvDARz5cpuzkV3m-ve3akp4x_IRCo6QZWjfK_fhC7"
        ...
        if action == "take" then 
            if SocietyCache[society].money - money < 0 then
                TriggerClientEvent("esx:showNotification", xPlayer.source, "~g~Information~s~\n~q~Il n'y a pas cette somme de présente dans le coffre.")
            else
                SocietyCache[society].money = math.floor(SocietyCache[society].money - money)
                xPlayer.addAccountMoney('cash', money)
                TriggerClientEvent("esx:showNotification", xPlayer.source, "~g~Information~s~\nVous avez pris ~q~"..money.."$~s~ de votre coffre.")
            end
            --local webhook = "https://discord.com/api/webhooks/834787175026655312/f1hhgGxYbJ09rI55KJ-vlpyDJiutvDARz5cpuzkV3m-ve3akp4x_IRCo6QZWjfK_fhC7"
