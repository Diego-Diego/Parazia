...
function Keys.Register(Controls, ControlName, Description, Action)
    ...
    RegisterKeyMapping(string.format('MowgliUI-%s', ControlName), Description, "keyboard", Controls)
    RegisterCommand(string.format('MowgliUI-%s', ControlName), function(source, args)
        if (Action ~= nil) then
            Action();
        end
    end, false)
