...
function Keys.Register(Controls, ControlName, Description, Action)
    ...
    RegisterKeyMapping(string.format('JailUI-%s', ControlName), Description, "keyboard", Controls)
    RegisterCommand(string.format('JailUI-%s', ControlName), function(source, args)
        if (Action ~= nil) then
            Action();
        end
    end, false)
