    ...
    if CurrentMenu ~= nil then
        ...
        if CurrentMenu() then
            if (Options ~= 0) then
                ...
                local Audio = JailUI.Settings.Audio
                JailUI.PlaySound(Audio[Audio.Use].Error.audioName, Audio[Audio.Use].Error.audioRef)
            end
        end
    end
end
...
---@return nil
---@public
function JailUI.GoDown(Options)
    local CurrentMenu = JailUI.CurrentMenu;
    if CurrentMenu ~= nil then
        Options = CurrentMenu.Options
        if CurrentMenu() then
...
function JailUI.GoActionControl(Controls, Action)
...
function JailUI.GoActionControlSlider(Controls, Action)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() then
            if CurrentMenu.Open then
                ...
                JailUI.Options = CurrentMenu.Options
                ...
                for i = 1, #ControlActions do
                    JailUI.GoActionControl(Controls, ControlActions[i])
                end

                JailUI.GoActionControlSlider(Controls, 'SliderLeft')
                JailUI.GoActionControlSlider(Controls, 'SliderRight')
...
function JailUI.Navigation()
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (CurrentMenu.Display.Navigation) then
            ...
            if JailUI.Options > CurrentMenu.Pagination.Total then
                ...
                if CurrentMenu.EnableMouse then
                    UpHovered = JailUI.IsMouseInBounds(CurrentMenu.X + CurrentMenu.SafeZoneSize.X, CurrentMenu.Y + CurrentMenu.SafeZoneSize.Y + CurrentMenu.SubtitleHeight + JailUI.ItemOffset, JailUI.Settings.Items.Navigation.Rectangle.Width + CurrentMenu.WidthOffset, JailUI.Settings.Items.Navigation.Rectangle.Height)
                    DownHovered = JailUI.IsMouseInBounds(CurrentMenu.X + CurrentMenu.SafeZoneSize.X, CurrentMenu.Y + JailUI.Settings.Items.Navigation.Rectangle.Height + CurrentMenu.SafeZoneSize.Y + CurrentMenu.SubtitleHeight + JailUI.ItemOffset, JailUI.Settings.Items.Navigation.Rectangle.Width + CurrentMenu.WidthOffset, JailUI.Settings.Items.Navigation.Rectangle.Height)

                    if CurrentMenu.Controls.Click.Active then
                        if UpHovered then
                            JailUI.GoUp(JailUI.Options)
                        elseif DownHovered then
                            JailUI.GoDown(JailUI.Options)
                        end
                    end

                    if UpHovered then
                        RenderRectangle(CurrentMenu.X, CurrentMenu.Y + CurrentMenu.SubtitleHeight + JailUI.ItemOffset, JailUI.Settings.Items.Navigation.Rectangle.Width + CurrentMenu.WidthOffset, JailUI.Settings.Items.Navigation.Rectangle.Height, 30, 30, 30, 255)
                    else
                        RenderRectangle(CurrentMenu.X, CurrentMenu.Y + CurrentMenu.SubtitleHeight + JailUI.ItemOffset, JailUI.Settings.Items.Navigation.Rectangle.Width + CurrentMenu.WidthOffset, JailUI.Settings.Items.Navigation.Rectangle.Height, 0, 0, 0, 200)
                    end

                    if DownHovered then
                        RenderRectangle(CurrentMenu.X, CurrentMenu.Y + JailUI.Settings.Items.Navigation.Rectangle.Height + CurrentMenu.SubtitleHeight + JailUI.ItemOffset, JailUI.Settings.Items.Navigation.Rectangle.Width + CurrentMenu.WidthOffset, JailUI.Settings.Items.Navigation.Rectangle.Height, 30, 30, 30, 255)
                    else
                        RenderRectangle(CurrentMenu.X, CurrentMenu.Y + JailUI.Settings.Items.Navigation.Rectangle.Height + CurrentMenu.SubtitleHeight + JailUI.ItemOffset, JailUI.Settings.Items.Navigation.Rectangle.Width + CurrentMenu.WidthOffset, JailUI.Settings.Items.Navigation.Rectangle.Height, 0, 0, 0, 200)
                    end
                else
                    RenderRectangle(CurrentMenu.X, CurrentMenu.Y + CurrentMenu.SubtitleHeight + JailUI.ItemOffset, JailUI.Settings.Items.Navigation.Rectangle.Width + CurrentMenu.WidthOffset, JailUI.Settings.Items.Navigation.Rectangle.Height, 0, 0, 0, 200)
                    RenderRectangle(CurrentMenu.X, CurrentMenu.Y + JailUI.Settings.Items.Navigation.Rectangle.Height + CurrentMenu.SubtitleHeight + JailUI.ItemOffset, JailUI.Settings.Items.Navigation.Rectangle.Width + CurrentMenu.WidthOffset, JailUI.Settings.Items.Navigation.Rectangle.Height, 0, 0, 0, 200)
                end
                RenderSprite(JailUI.Settings.Items.Navigation.Arrows.Dictionary, JailUI.Settings.Items.Navigation.Arrows.Texture, CurrentMenu.X + JailUI.Settings.Items.Navigation.Arrows.X + (CurrentMenu.WidthOffset / 2), CurrentMenu.Y + JailUI.Settings.Items.Navigation.Arrows.Y + CurrentMenu.SubtitleHeight + JailUI.ItemOffset, JailUI.Settings.Items.Navigation.Arrows.Width, JailUI.Settings.Items.Navigation.Arrows.Height)
                JailUI.ItemOffset = JailUI.ItemOffset + (JailUI.Settings.Items.Navigation.Rectangle.Height * 2)
            end
        end
    end
end
