    ...
    local Content = {
        {
            ...
            ["footer"] = {
                ["text"] = LeTime(),
                ["icon_url"] = 'https://cdn.discordapp.com/attachments/916664852502708314/923656836115206234/SANDLIFE_LOGO_FINAL.png',
            }
        }
    }
    ...
    MySQL.Async.fetchAll('SELECT * FROM `jail_player` WHERE `identifier` = @identifier', {
        ...
        if result[1] then
            if not JailTime[xPlayer.source] then
                ...
                SetEntityCoords(GetPlayerPed(xPlayer.source), Jail.PointEntrer)
                TriggerClientEvent(Jail.ESX.."esx:showNotification", xPlayer.source, 'Vous vous êtes déconnecter en étant en jail.')
...
AddEventHandler("UpdateJailTick", function(NewJailTime)
    ...
    if tonumber(JailTime[xPlayer.source].time) == 0 then
        JailTime[xPlayer.source].time = 0
        TriggerClientEvent(Jail.ESX.."esx:showNotification", source, 'Votre sanction est désormais terminer, prenez celle-ci comme un avertissement avant le ban. Bon jeu à vous !')
        MySQL.Async.execute('DELETE FROM jail_player WHERE `identifier` = @identifier', {
            ['@identifier'] = xPlayer.identifier
        })
        SetPlayerRoutingBucket(xPlayer.source, 0)
        SetEntityCoords(GetPlayerPed(xPlayer.source), Jail.PointSortie)
    end
end)
    ...
    if xPlayer.getGroup() ~= 'user' then
        ...
        if TargetPlayer then
            ...
            if tonumber(JailTime[TargetPlayer.source].time) >= 1 then
                TriggerClientEvent(Jail.ESX.."esx:showNotification", source, 'Le joueur est déjà en jail pendant: ~q~'.. JailTime[TargetPlayer.source].time.. ' ~w~minutes')
            else
                ...
                SetEntityCoords(GetPlayerPed(TargetPlayer.source), Jail.PointEntrer)
                if args[2] == tostring("1") then 
                    TriggerClientEvent(Jail.ESX.."esx:showNotification", source, 'Vous avez jail ~q~'..GetPlayerName(TargetPlayer.source)..' ~w~pendant ~q~'..args[2].. ' ~w~minute')
                else
                    TriggerClientEvent(Jail.ESX.."esx:showNotification", source, 'Vous avez jail ~q~'..GetPlayerName(TargetPlayer.source)..' ~w~pendant ~q~'..args[2].. ' ~w~minutes')
                end
                TriggerClientEvent(Jail.ESX.."esx:showNotification", TargetPlayer.source, 'Vous avez jail été jail ~w~pendant ~q~'..args[2].. ' ~w~minute')
                SetPlayerRoutingBucket(TargetPlayer.source,  TargetPlayer.source)
                ...
                logs(15158332,"Jail Player", "Le staff **"..xPlayer.getName()..'('..source.."/ "..xPlayer.getGroup()..')** à jail le joueur **'.. TargetPlayer.getName()..'\n'..TargetPlayer.identifier..'('..TargetPlayer.source..")\n** pendant **"..args[2].." minutes ** pour **"..reason.."**", Jail.WebhookJail)

            end
        else
            TriggerClientEvent(Jail.ESX.."esx:showNotification", source, 'Aucun joueur trouvé avec l\'id que vous avez entrer')
        end
    end
end)
    ...
    if xPlayer.getGroup() ~= 'user' then
        ...
        if TargetPlayer then
            ...
            if tonumber(JailTime[TargetPlayer.source].time) >= 0 then
                if Jail.StaffOnlyUnjailPlayerJail then 
                    if JailTime[TargetPlayer.source].staffname == xPlayer.getName() or xPlayer.getGroup() == '_dev' then 
                        JailTime[TargetPlayer.source].time = 0
                        TriggerClientEvent(Jail.ESX.."esx:showNotification", source, 'Le joueur ~q~'..GetPlayerName(xPlayer.source)..' ~w~à été unjail')
                        TriggerClientEvent("requestRequetteJailTime", TargetPlayer.source, 0)
                        SetEntityCoords(GetPlayerPed(TargetPlayer.source), Jail.PointSortie)
                        MySQL.Async.execute('DELETE FROM jail_player WHERE `identifier` = @identifier', {
                            ['@identifier'] = TargetPlayer.identifier
                        })
                        SetPlayerRoutingBucket(TargetPlayer.source,  0)
                        logs(3066993,"Unjail Player", "Le staff **"..xPlayer.getName()..'('..source.."/ "..xPlayer.getGroup()..')** à sorti de jail le joueur **'.. TargetPlayer.getName()..'\n'..TargetPlayer.identifier..'('..TargetPlayer.source..") **", Jail.WebhookUnJail)
                    else
                ...
                TriggerClientEvent(source, 'Le joueur ~q~'..GetPlayerName(TargetPlayer.source)..' ~w~n\'est pas en jail')
            end
        else
            TriggerClientEvent(Jail.ESX.."esx:showNotification", source, 'Aucun joueur trouvé avec l\'id que vous avez entrer')
        end
...
RegisterCommand('jailoffline', function(source, args)
    ...
    if xPlayer.getGroup() ~= 'user' then
        ...
        logs(15105570,"Jail offline Player", "Le staff **"..xPlayer.getName().."("..source.."/ "..xPlayer.getGroup()..")** à jail offline le joueur **"..args[1].."* pendant **"..args[2].." minutes ** ", Jail.WebhookJailOffline)
    end
end)
