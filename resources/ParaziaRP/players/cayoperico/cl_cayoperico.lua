...
DrawDistanceIsland = 20.0

ActivationDistanceScaler = 1.2
...
local function CreateNewBlips()
    ...
    for _, location in pairs(TeleportLocationsIsland) do
        local blip = nil
        local blipName = BlipIsland.LosSantosName
        if not IsOnIsland then
            ...
            blipName = BlipIsland.IslandName
        end

        if blip then
            SetBlipAsShortRange(blip, BlipIsland.MinimapOnly)
            SetBlipSprite(blip, BlipIsland.Sprite)
            SetBlipColour(blip, BlipIsland.Color)
            SetBlipScale(blip, BlipIsland.Size)
...
local function GetClosestLocation()
    local closestLocation = TeleportLocationsIsland[1]
    local closestLocationCoords = GetFromCoordinate(closestLocation)

    for index, location in pairs(TeleportLocationsIsland) do
    ...
    if #(PedCoordinates - closestLocationCoords) < DrawDistanceIsland then
...
Citizen.CreateThread(function()
    ...
    AddTextEntry("DOPE_PERICO_HELP",
                 "Appuyez sur ~a~ pour payer votre billet ~g~100$~s~ vers ~q~~a~~s~")
    CreateNewBlips()
    while true do
        Wait(waitTime)
        ...
        if not IsInSideTeleportLocation then
            ...
            if distance > 20.0 then
                ...
                if distance < (ActivationDistanceScaler) then
      ...
      Citizen.CreateThread(
        function()
          if not IsPlayerTeleportActive() then
            ...
            if (Cutscenes.enabled) then BeginLeaving(IsOnIsland) end
              
            if (Cutscenes.enabled) then BeginLanding(IsOnIsland) end
