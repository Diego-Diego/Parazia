...
AddEventHandler("ronflex:creator", function(player)
    ...
    while main do
        ...
        RageUI.IsVisible(main, function()
            ...
            if cm then
                RageUI.Separator("→ Prénom : ~q~"..prenom)
                RageUI.Separator("→ Nom : ~q~"..lastname)
                RageUI.Separator("→ Âge : ~q~"..naissance)
                RageUI.Separator("→ Taille : ~q~"..cm)
                RageUI.Separator("↓ ~g~MERCI DE CHOISIR SON SEXE ~s~↓")
            end
        ...
        while true do 
            ...
            if dist < 5 then
                ...
                if IsControlJustPressed(1,51) then
                    ...
                    TriggerServerEvent("ronflex:bucket", false)
                    ESX.ShowAdvancedNotification("~q~ParaziaRP", "Création d'identité", "Bienvenue sur ParaziaRP\nProfitez bien de votre aventure !", "CHAR_SEALIFE", 8)
