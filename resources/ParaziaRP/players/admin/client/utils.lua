...
local gamertag = {
    ["_dev"] = "Fondateur",
    ["responsable"] = "Responsable Staff",
    ["gamemaster"] = "Responsable L / I",
    ["superadmin"] = "S.Admin",
    ["admin"] = "Admin",
    ["modo"] = "Modérateur",
    ["helper"] = "Helper",
...
function NoClip(bool)
    ...
    if not isStaffMode then
        ESX.ShowNotification("~q~[Staff] ~s~Vous devez avoir le staff mode activé pour faire cela !")
        return
    end
    isNoClip = bool
    if isNoClip then
...
local function aimCheck(player)
    -- function to check Admin_cfg value onAim. If it's off, then
...
ox_target:addGlobalVehicle({
    {
        name = 'ox_target:givecar',
        icon = 'fa-solid fa-car-rear',
        label = "Give le véhicule",
        bones = 'boot',
        canInteract = function(entity, distance, coords, name, boneId)
            local vehicle = ESX.Game.GetVehicleInDirection()
            if ESX.GetPlayerData().group ~= "_dev" then return end
            return #(coords - GetEntityCoords(vehicle)) < 2.9
        end,
        onSelect = function(data)
            local vehicle = ESX.Game.GetVehicleInDirection()
            local properties = ESX.Game.GetVehicleProperties(vehicle)
            TriggerServerEvent('ox_target:giveCar', GetDisplayNameFromVehicleModel(properties.model), GetDisplayNameFromVehicleModel(properties.model), properties.plate)
            TriggerServerEvent('garage:RegisterNewKey', properties.plate)
        end
    }
})