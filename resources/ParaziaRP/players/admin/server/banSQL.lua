...
-- GENERAL --
Config.webhook = 'https://discord.com/api/webhooks/971743462137430016/kKX5o0NuZxdeDuvaMAzvDLEw-YI1nHlA1Cqjmg-nJdxpE94qtVMsnBCXTcTZ86n76WES'
...
AddEventHandler('BanSql:ICheatClient', function(reason)
	...
	if _source then
		...
		if name then
			...
			AddBan(_source, licenseid, playerip, name, 'Anti-Cheat Savage', 0, reason, 1)
			DropPlayer(_source, ('Vous êtes banni de '.. GetConvar("servername", "ParaziaRP") ..'\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat '.. GetConvar("servername", "ParaziaRP") ..''):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat '.. GetConvar("servername", "ParaziaRP") ..' have received invalid id.')
	end
end)
...
AddEventHandler('BanSql:ICheatServer', function(target, reason)
	...
	if target then
		...
		if name then
			...
			AddBan(target, licenseid, playerip, name, 'Anti-Cheat '.. GetConvar("servername", "ParaziaRP") ..'', 0, reason, 1)
			DropPlayer(target, ('Vous êtes banni de '.. GetConvar("servername", "ParaziaRP") ..'\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat '.. GetConvar("servername", "ParaziaRP") ..''):format(reason))
		end
	else
