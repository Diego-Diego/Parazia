...
AddEventHandler("adminmenu:takeReport", function(reportId)
    ...
    if not reportsTable[reportId] then
        TriggerClientEvent("esx:showNotification", source, "~q~[Report] \n\n~s~Ce report n'est plus en attente de prise en charge")
        return
    end
    reportsTable[reportId].takenBy = GetPlayerName(source)
    reportsTable[reportId].taken = true
    if players[reportId] ~= nil then
        TriggerClientEvent("esx:showNotification", reportId, "~q~[Report] \n\n~s~Votre report a été pris en charge.")
    end
    notifyActiveStaff("~q~[Report] \n\n~s~Le staff ~q~"..GetPlayerName(source).."~s~ a pris en charge le report ~y~n°"..reportsTable[reportId].uniqueId)
...
AddEventHandler("adminmenu:closeReport", function(reportId)
    ...
    notifyActiveStaff("~q~[Report] \n\n~s~Le staff ~q~"..GetPlayerName(source).."~s~ a ~g~cloturé ~s~le report ~y~n°"..reportsTable[reportId].uniqueId)
...
RegisterCommand("report", function(source, args)
    ...
    notifyActiveStaff("~q~[Report] \n\n~s~Un nouveau report a été reçu. ID Unique: ~y~" .. reportsCount)
