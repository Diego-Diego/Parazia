...
ESX.AddGroupCommand('annonce', "_dev", function(source, args, user)
	local message = table.concat(args, ' ')
	local xPlayers = ESX.GetPlayers()
	for i = 1, #xPlayers, 1 do
		local thePlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Annonce Staff', 'ParaziaRP', message , 'CHAR_SOVALIFE')
	end
end, {help = "Announce a message to the entire server", params = {
	{name = "announcement", help = "The message to announce"} 
}})]]