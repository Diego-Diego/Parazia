...
RegisterNetEvent('TEST:process_checkout_case')
AddEventHandler('TEST:process_checkout_case', function(type)
    local source = source;
    if (source) then
        local identifier = GetIdentifiers(source);
        local xPlayer = ESX.GetPlayerFromId(source)
...
Citizen.CreateThread(function()
    ...
    for k,v in pairs(MysteryBox.ListBox) do
        ESX.RegisterUsableItem(k, function(source)
            ...
            TriggerClientEvent('closeall', source)
            Wait(100)
            TriggerEvent('TEST:process_checkout_case_item', source, k, MysteryBox.ListBox[k].label)
        end)
    end
end)

RegisterNetEvent('TEST:process_checkout_case_item')
AddEventHandler('TEST:process_checkout_case_item', function(src, type, label)
    if (src) then
