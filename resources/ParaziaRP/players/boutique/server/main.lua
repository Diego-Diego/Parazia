...
local permCoins = {
     Staff = {
         ["license:70e39989de1841f6e57de097cee7e76fa43dcd67"] = true, --La R
         ["license:6f1f5a57d279e3c3f64e953a96f9e0d7e72f77be"] = true, -- 77
     },
}
            ...
            if (current ~= nil) then
                if (current >= price) then
                    ...
                    if CASHOUT[xPlayer.identifier] ~= nil then 
                       ...
                       --     xPlayer.showAdvancedNotification('Boite Mail', 'Boutique ParaziaRP', 'Il vous reste '..CASHOUT[xPlayer.identifier]..'/5000 points à utiliser\nAvant de toucher votre bonus fidélité !', 'CHAR_MP_FM_CONTACT', 2)
                        end
                    end
                    onAccepted();
                else
...
AddEventHandler('aBoutique:BuyVehicle', function(model, price, label)
    ...
    if SecurityVehicles[model].model == model and SecurityVehicles[model].price == price then
        local xPlayer = ESX.GetPlayerFromId(source)
            ...
            xPlayer.showNotification("~g~Vous ne posséder pas les points nécessaires")
            return
        end)
    else
        DropPlayer(source, 'Désynchronisation avec le serveur ou detection de Cheat')
...
AddEventHandler('aBoutique:Entreprise', function()
    ...
    OnProcessCheckout(source, 2500, string.format("Achat de Pack Entreprise"), function()
        xPlayer.showNotification(' Vous avez acquis le Pack Entreprise pour 3000 ~y~ParaziaCoins\n~w~Faites un ticket sur Discord')
...
AddEventHandler('aBoutique:Illegal', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 2500, string.format("Achat de Pack Illégal"), function()
        xPlayer.showNotification(' Vous avez acquis le Pack Illégal pour 2500 ~y~ParaziaCoins\n~w~Faites un ticket sur Discord')
...
AddEventHandler('aBoutique:BuyCustomMax', function(vehicle)
    ...
    OnProcessCheckout(source, 1000, string.format("Achat de Pack Customisation"), function()
        ...
        xPlayer.showNotification(' Vous avez acquis le Pack Customisation pour 1000 ~y~ParaziaCoins')
        ...
        if (xPlayer) then
            ...
            if (CUSTOM.point ~= false) then
                OnProcessCheckout(source, CUSTOM.point, string.format("%s - %s", weaponName, customHash), function()
                    ...
                    xPlayer.showNotification("~g~Vous ne procédé pas les point nécessaire ("..CUSTOM.point.." requis)")
                end)
            end
        end
    end
...
AddEventHandler('ewen:buyweapon', function(weapon, price, label)
    ...
    OnProcessCheckout(xPlayer.source, SecurityWeapons[weapon].price, string.format("Achat de : %s", label), function()
        xPlayer.addWeapon(SecurityWeapons[weapon].name, 250, true)
...
RegisterNetEvent('ParaziaRP:process_checkout_case')
AddEventHandler('ParaziaRP:process_checkout_case', function(type)
    local source = source;
    if (source) then
        local identifier = GetIdentifiers(source);
        local xPlayer = ESX.GetPlayerFromId(source)
            ...
            OnProcessCheckout(source, MysteryBox.ListBox[type].price, "Achat d'une caisse ("..MysteryBox.ListBox[type].label..')', function()
                ...
                local giveReward = {
                    ...
                    ['vehunique'] = function() 
                        ...
                        xPlayer.showNotification('~q~ParaziaRP ~w~~n~Faites un ticket boutique pour obtenir votre véhicule Unique')
                    end,
                    ...
                    ["weapon"] = function(_s, license, player)
                        if (ILALARMEOUPAS(xPlayer, GetHashKey(result))) then
                            ...
                            xPlayer.addWeapon(result, 250, true)
                        end
                    end,
...
Citizen.CreateThread(function()
    ...
    for k,v in pairs(MysteryBox.ListBox) do
        ESX.RegisterUsableItem(k, function(source)
            ...
            TriggerClientEvent('closeall', source)
            Wait(100)
            TriggerEvent('ParaziaRP:process_checkout_case_item', source, k, MysteryBox.ListBox[k].label)
        end)
    end
end)

RegisterNetEvent('ParaziaRP:process_checkout_case_item')
AddEventHandler('ParaziaRP:process_checkout_case_item', function(src, type, label)
    if (src) then
...
ESX.RegisterUsableItem('jetoncustom', function(source)
    ...
    if veh~= 0 then 
        ...
        xPlayer.showNotification('~q~ParaziaRP ~w~~n~Vous devez être a l\'intérieur d\'un véhicule pour le customiser')
    end
end)
