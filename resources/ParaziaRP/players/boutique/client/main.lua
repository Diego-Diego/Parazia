...
function OpenMenuMain()
    ...
    while menu ~= nil do
        RageUI.IsVisible(menu, function()
            ...
            RageUI.Separator('Vos ~g~Parazia~w~Coins : ~q~'..SeaCoins)
        ...
        RageUI.IsVisible(voitures, function()
            for k,v in pairs(BoutiqueVehicles) do
                RageUI.List(v.label..' | Prix : ~g~'..v.price, Action, index.list, nil, {}, true, {
                    onActive = function()
        ...
        RageUI.IsVisible(avionhelico, function()
            for k,v in pairs(BoutiqueAirPlaines) do 
                RageUI.List(GetLabelText(v.model)..' | Prix : ~g~'..v.price, Action, index.list, nil, {}, true, {
                    onActive = function()
                        FreezeEntityPosition(PlayerPedId(), true)
                        SetEntityVisible(PlayerPedId(), false, 0)
                        SetWeatherTypeNow('EXTRASUNNY')
            ...
            if (selected) then
                if (ESX.Table.SizeOf(selected) > 0) then
                    for i, v in pairs(selected.components) do
                        RageUI.Button(v.label, nil, { RightLabel = "~g~Acheter ~w~→250" }, true, {
                            onSelected = function()
                                TriggerServerEvent('tebex:on-process-checkout-weapon-custom', selected.name, v.hash)
                                ESX.ShowNotification('Vous avez acheter '..v.label.. ' pour 250 ~q~ParaziaRPCoins')
                            end,
                        })
                    end
                else
                ...
                if v.model ~= 'caisse_fidelite' then
                    RageUI.List(v.label..' | Prix : ~g~'..v.price, Action, index.list, nil, {}, true, {
                        ...
                        onSelected = function()
                            ...
                            elseif Button == 2 then
                                RageUI.CloseAll()
                                TriggerServerEvent('ParaziaRP:process_checkout_case', v.model)
                            end
                        end
                    })
                else
...
function OpenMenuPreviewCaisse(model, label)
    ...
    while CaissePreview do
        ...
        RageUI.IsVisible(CaissePreview, function()
            ...
            RageUI.Separator('~g~ARME NON PERMA~s~')
