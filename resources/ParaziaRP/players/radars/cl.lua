		...
		for RadarId,Radar in pairs(Radars.Radars) do
			if Radar.PropPos and #(playerCoords - Radar.PropPos) < 40 then
				...
				if not Radar.Destroyed then
					...
					--Draw3DText(Radar.PropPos - vector3(0, 0, 0.3), '~g~Radar détruit !', 1.0)
					ESX.Game.Utils.DrawText3D(Radar.PropPos - vector3(0, 0, 0.3), '~g~Radar détruit !', 1.0)
				end
			end
		end
		Citizen.Wait(Timer)
		...
		if IsPedInAnyVehicle(PlayerPedId()) then
			for RadarId,Radar in pairs(Radars.Radars) do
				...
				elseif #(playerCoords - Radar.Pos) < Radar.Range * 2 and #blips > 0 and GetCurrentKMHSpeed() > Radar.MinSpeed then
					ESX.ShowNotification('~g~Attention zone de radar (' .. Radar.MinSpeed .. 'km/h) !')
				end
			end
		else
			Citizen.Wait(3000)
