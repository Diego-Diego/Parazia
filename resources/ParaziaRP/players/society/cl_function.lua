...
mSociety.InitMowgliUIMenu = function(_title, _subtitle, _texturedict, _texturename, _rgb, _banner)
    if _banner then
	    RMenu.Add('bossmenu', 'main', MowgliUI.CreateMenu(_title, _subtitle, nil, nil, _texturedict, _texturename))
    else
        RMenu.Add('bossmenu', 'main', MowgliUI.CreateMenu(_title, _subtitle, 10, 200))
    end
	if _rgb ~= nil then
		RMenu:Get('bossmenu', 'main'):SetRectangleBanner(_rgb[1], _rgb[2], _rgb[3], 500)
	end
    RMenu.Add('bossmenu', 'manage_employees', MowgliUI.CreateSubMenu(RMenu:Get('bossmenu', 'main'), _title, _subtitle))
    RMenu.Add('bossmenu', 'update_employee', MowgliUI.CreateSubMenu(RMenu:Get('bossmenu', 'manage_employees'), _title, _subtitle))
    RMenu.Add('bossmenu', 'manage_salary', MowgliUI.CreateSubMenu(RMenu:Get('bossmenu', 'main'), _title, _subtitle))
					...
					if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == v.name and ESX.PlayerData.job.grade_name == "boss" then
						if #(pCoords - v.pos) <= 10.0 then
							...
							if #(pCoords - v.pos) <= 1.5 then
                                Draw3DText(v.pos.x, v.pos.y, v.pos.z, "Appuyez sur ~q~E~s~ pour intéragir")
								if IsControlJustPressed(0, 51) then
									mSociety.OpenMowgliUIMenu(v, v.options)
								end
							end
						end
					end
...
AddEventHandler("mSociety:SendRequestRecruit", function(bb, cc, pp)
	RageUI.PopupChar({
		message = "~q~infos:~s~\n"..bb.."\n\n~q~Y~s~ "..mSociety.Trad["accept"]..". | ~q~G~s~ "..mSociety.Trad["decline"]..".",
