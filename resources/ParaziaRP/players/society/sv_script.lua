...
mSociety.GetPlayerDetails = function(src)
	local player_id = src
	local ids = mSociety.ExtractIdentifiers(player_id)
	if mSocietyLOG.Discord then if ids.discord ~= "" then _discordID ="**Discord:** "..ids.discord.." <@" ..ids.discord:gsub("discord:", "")..">" else _discordID = "**Discord:** N/A" end else _discordID = "" end
	if mSocietyLOG.Steam then  if ids.steam ~= "" then _steamID = "**SteamID:** ["..ids.steam.."](https://steamcommunity.com/profiles/" ..tonumber(ids.steam:gsub("steam:", ""),16)..")" else _steamID = "**SteamID:** N/A" end else _steamID = "" end
	if mSocietyLOG.License then if ids.license ~= "" then _license ="**License:** " ..ids.license else _license = "**License :** N/A" end else _license = "" end
	if mSocietyLOG.Ip then if ids.ip ~= "" then _ip = "**IP:** [||"..ids.ip:gsub("ip:", "").."||](https://www.ip-tracker.org/locator/ip-lookup.php?ip=" ..ids.ip:gsub("ip:", "")..")" else _ip = "**IP :** N/A" end else _ip = "" end
	return _steamID..' \n'.. _discordID..' \n'.._license..' \n'.._ip
end
