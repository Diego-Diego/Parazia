...
function OpenAFKMenu()
    if #(GetEntityCoords(PlayerPedId()) - vector3(1642.719, 2570.633, 51.516)) < 250 then
        ESX.ShowNotification('Vous ne pouvez pas faire cette commande en étant en Jail.')
    else
        if GetSafeZone() then
            local menu = RageUI.CreateMenu("", "Gagne de l'argent en étant AFK") 
            ...
            ESX.ShowNotification('Vous devez être en Zone Safe pour utiliser le /invest')
        end
    end
end
    ...
    while true do
        if InAfkZone then
            if AfkTime >= 1 then
                ...
                DrawMissionText('~w~Vous êtes dans la ~q~Zone Investissement~w~ !\nVotre investissement est terminer dans ~q~'..AfkTime..' ~w~minutes', 100)
                InAFK = true
            else
                InAFK = false
            end
