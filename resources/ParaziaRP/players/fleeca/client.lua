...
function Fleeca:AnimStartFin(PEqsd)
    ...
    if PEqsd == 1 then
        ...
        ESX.ShowNotification("~q~Votre tentative de piratage a échoué.")
    elseif PEqsd == 2 then
        ClearPedTasks(pPed)
        ESX.ShowNotification("~q~Vous avez abandonné.")
    end
...
function Fleeca:StartFleecaHeist()
    ...
    if not FleecaIn then
        ESX.ShowNotification("~q~Vous n'êtes pas dans une Fleeca Bank.")
        return
    end
