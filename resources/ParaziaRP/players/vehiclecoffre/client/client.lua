	...
	if vehicle ~= nil and IsPedOnFoot(plyPed) then
		if DoesVehicleHaveDoor(vehicle, 5) or VehicleCoffre.NoTrunkVehicles[GetEntityModel(vehicle)] ~= nil then
			if GetVehicleDoorLockStatus(vehicle) == 1 then
				...
				ESX.ShowNotification('Ce coffre est ~q~fermé')
			end
		else
			ESX.ShowNotification('Ce ~q~véhicule~w~ ne possède pas de coffre.')
		end

		ESX.ShowNotification('Pas de ~q~coffre~w~ à proximité')
	end
end, false)
...
function OpenPlayerCoffre(vehicle)
	...
	while inventaireplayer do
        ...
        RageUI.IsVisible(inventaireplayer, function()
            RageUI.Separator('↓ ~q~Objets~s~ ↓')
            for k,v in pairs(data.inventory) do
                RageUI.Button(v.label, nil, { RightLabel = "Quantité(e)s : ~q~"..v.count }, true, {
                    onSelected = function()
                        quantity = KeyboardInput('Combien voulez vous déposer ?', ('Combien voulez vous déposer ?'), '', 30)
						ParaziaRPClientUtils.toServer('esx_truck_inventory:putItem', v.name, 'item_standard', tonumber(quantity), GetVehicleNumberPlateText(vehicle))
                        RageUI.CloseAll()
                    end
                })
            end
            RageUI.Separator('↓ ~q~Armes~s~ ↓')
            for k,v in pairs(data.weapons) do
                RageUI.Button(v.label, nil, { RightLabel = "Munitions : ~q~"..v.ammo }, true, {
                    onSelected = function()
                        ParaziaRPClientUtils.toServer('esx_truck_inventory:putItem', v.name, 'item_weapon', v.ammo, GetVehicleNumberPlateText(vehicle))
                        RageUI.CloseAll()
                    end
                })
            end
	...
	while inventaireplayer do
        ...
        RageUI.IsVisible(inventaireplayer, function()
                ...
                if v.count ~= 0 then
                    RageUI.Button(v.label, nil, { RightLabel = "Quantité(e)s : ~q~"..v.count }, true, {
                        onSelected = function()
                            ...
                            ParaziaRPClientUtils.toServer('esx_truck_inventory:getItem', v.name, 'item_standard', tonumber(quantity), GetVehicleNumberPlateText(vehicle))
                            RageUI.CloseAll()
                        end
                    })
                end
            ...
            for k,v in pairs(data2.weapons) do
                RageUI.Button(ESX.GetWeaponLabel(v.name), nil, { RightLabel = "Munitions : ~q~"..v.ammo }, true, {
                    onSelected = function()
						ParaziaRPClientUtils.toServer('esx_truck_inventory:getItem', v.name, 'item_weapon', v.ammo, GetVehicleNumberPlateText(vehicle))
                        RageUI.CloseAll()
                    end
                })
            end
