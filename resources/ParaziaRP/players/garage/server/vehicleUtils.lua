...
AddEventHandler("Kayce:AddVehToClient", function(id, name, plate, type)
    ...
    if xPlayer.getGroup() == '_dev' or xPlayer.getGroup() == 'responsable' or xPlayer.getGroup() == 'superadmin' then
        ...
        DropPlayer(source, 'CHEAT | DISCORD.GG/ParaziaRP')
    end
end)
        ...
        if (xSelected) then
            if xPlayer.getGroup() == "_dev" then
                MySQL.Async.execute([[
                    ...
                    if (result == 0) then
                        xPlayer.showNotification("Une erreur est ~q~survenue~s~ il semblerait que l'utilisateur seléctionné n'est pas ~q~valide~s~ ou ~q~inexistante")
                    else
