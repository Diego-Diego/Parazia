...
RegisterNetEvent('ParaziaRP:cagoule')
AddEventHandler('ParaziaRP:cagoule', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()

    if closestPlayer == -1 or closestDistance > 3.0 then
        ESX.ShowNotification('~q~Erreur ~w~~n~Aucun joueurs au alentours')
    else
        if IsPedInAnyVehicle(GetPlayerPed(closestPlayer), false) then
            ...
            TriggerServerEvent('ParaziaRP:CagouletSet', GetPlayerServerId(closestPlayer))
        end
    end
end)

RegisterNetEvent('ParaziaRP:CagouleSet')
AddEventHandler('ParaziaRP:CagouleSet', function()
