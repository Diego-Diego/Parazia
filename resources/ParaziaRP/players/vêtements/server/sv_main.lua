...
RegisterNetEvent("ronflex:addtenueitem", function(label, skin)
    ...
    if NumberCount+1 > NumberTenueAutorized then 
        ...
        if Account == 'nomoney' then
            xPlayer.showNotification('~q~ParaziaRP ~w~~n~Vous n\'avez pas assez d\'argent sur vous')
        else
            ...
            xPlayer.showNotification(' Vous avez crée une tenue (~q~'..label..'~w~)')
...
RegisterNetEvent("ronflex:paidaccesoires", function(type, name, skin)
    ...
    if xPlayer.getAccount('cash').money >= 300 then 
        ...
        TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
        xPlayer.showNotification("~q~ParaziaRP~s~~n~Vous venez d'acheter un "..type.."")
    else
        xPlayer.showNotification("~q~ParaziaRP~s~~n~Vous n'avez pas les fonds nécéssaires")
    end
end)
...
RegisterNetEvent('ronflex:donnertenue', function(player, id)
    ...
    if tPlayer ~= nil then
        if ClothesPlayer[xPlayer.identifier][id] then
            ...
            xPlayer.showNotification("~q~ParaziaRP~s~~n~ Vous avez donné votre tenue ")
            tPlayer.showNotification("~q~ParaziaRP~s~~n~ Vous avez reçu une tenue ")
...
RegisterNetEvent('ewen:RenameTenue', function(id, NewLabel)
    ...
    if ClothesPlayer[xPlayer.identifier][id] then
        if ClothesPlayer[xPlayer.identifier][id].identifier == xPlayer.identifier then
            xPlayer.showNotification(' Vous avez renommer votre tenue (~q~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
...
RegisterNetEvent('ronflex:deletetenue', function(id, NewLabel)
    ...
    if ClothesPlayer[xPlayer.identifier][id] then
        if ClothesPlayer[xPlayer.identifier][id].identifier == xPlayer.identifier then
            xPlayer.showNotification(' Vous avez supprimer votre tenue (~q~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
            ClothesPlayer[xPlayer.identifier][id] = nil
            TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
    ...
    if ClothesPlayer[xPlayer.identifier][id] then
        if ClothesPlayer[xPlayer.identifier][id].identifier == xPlayer.identifier then
            if typee == "equip" then 
                ...
                xPlayer.showNotification(' Vous avez équiper votre tenue (~q~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
                TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
            ...
            elseif typee == "deposit" then 
                ...
                xPlayer.showNotification(' Vous avez déposer votre tenue (~q~'..ClothesPlayer[xPlayer.identifier][id].label..'~w~)')
                TriggerClientEvent("ronflex:recieveclientsidevetement", xPlayer.source, ClothesPlayer[xPlayer.identifier])
            end
        else
            DropPlayer(source, 'Mhh c\'est chaud c\'que t\'essaie de faire')
