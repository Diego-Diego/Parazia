        ...
        while #GLOBAL_FISH > 0 do
            ...
            for i=1, #GLOBAL_FISH, 1 do
                ...
                if Vdist(GLOBAL_COORDS.x, GLOBAL_COORDS.y, GLOBAL_COORDS.z, coords.x, coords.y, coords.z) <= 2.0 then
                    DrawText3Ds(coords.x, coords.y, coords.z, "[~g~E~w~] - Pick")
...
TryToCatchFish = function()
    ...
    repeat
        Wait(10)
        local rnd = 1
        rnd = math.random(1, #Peche.FishTable)
        if math.random(100) <= Peche.FishTable[rnd].chance then
            foundFish = true
            fishNumber = rnd
        end
    until foundFish

    local rewardtype = Peche.FishTable[fishNumber]
    ClearPedTasks(GLOBAL_PED)
