...
local PaintBallShop =  {
	{name="PaintBall", id=73, x=-1095.74,y=-1267.34,z=5.85,h=210.07, typeM = "create"},
                ...
                if(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) < 1.0)then
                    if (incircle == false) then
						if v.typeM == "create" then
							DisplayHelpText("Press ~INPUT_CONTEXT~ Pour crée une partie.")
						else
							DisplayHelpText("Press sur ~INPUT_CONTEXT~ Pour rejoindre un partie.")
						end
                    end
                    incircle = true
            ...
            if(Vdist(pos.x, pos.y, pos.z, mapRedOutCoords.x, mapRedOutCoords.y, mapRedOutCoords.z) < 5.0)then
                ...
                if(Vdist(pos.x, pos.y, pos.z, mapRedOutCoords.x, mapRedOutCoords.y, mapRedOutCoords.z) < 1.0)then
                    if (incircleRed == false) then
						-- if v.typeM == "create" then
							DisplayHelpText("Press ~INPUT_CONTEXT~ Pour quitter la partie.")
						-- else
...
RegisterNUICallback('validate', function(data, cb)
	...
	if reason == "" then
		...
		notification("Partie crée ! Elle sera supprimée dans 3 minutes si elle n'est pas lancée","success")
...
function openMenuMain(data)
	...
	for k,v in pairs(allSession) do
		if v.CurStat == "WaitingPeople" then
			...
			newitem[k] = _menuPool:AddSubMenu(mainMenu, "Rejoindre "..tostring(v.creator).."'s lobby")
			
			EquipeBleu = v.EquipA
			EquipeRouge = v.EquipB
			
			blueitem[k] = UIMenuItem.New("Rejoindre Bleu ".. tostring(#EquipeBleu) .."/"..tostring(v.nbpersequip), "")
			newitem[k]:AddItem(blueitem[k])
			
			reditem[k] = UIMenuItem.New("Rejoindre Rouge ".. tostring(#EquipeRouge) .."/"..tostring(v.nbpersequip), "")
			newitem[k]:AddItem(reditem[k])
			
			if GetPlayerFromServerId(k) ~= -1 then
				if GetPlayerPed(GetPlayerFromServerId(k)) == PlayerPedId() then
					launchitem[k] = UIMenuItem.New("Lancez la partie", "Seul le créateur de la partie peut lancez !")
...
function endTheGame(winner)
	...
	isCurrentlyOutInGame = false
	if winner == "tie" then
		message = "~y~éQUIPE INCOMPLETE"
		message1 = "Les équipe sont pas compléte"
	else
		if winner == currentTeam then
			message = "~g~Bien joué !"
			message1 = "T'es bon, T'es bon !"
		else
			message = "~r~Perdu !"
			message1 = "Trop nul, va t'entrainer !"
		end
	end
